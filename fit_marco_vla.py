...
INDIR       = '.'
INFILENAME  = 'marco_vla.csv'
OUTDIR      = 'fits_vla'
OUTFILENAME = 'marco_fits_vla.csv'
DOPLOT      = True
...
usecols = ["ID_1", "primary", "secondary"] + flux_cols + flux_error_cols

marco = np.genfromtxt(INFILENAME, delimiter=',', names=True, usecols=usecols, dtype=None)

i=0

print >> outfile, "ID_1,S_150_1,e_S150_1,SpIndex_1,e_SpIndex_1,me1_1,S_150_2,e_S150_2,SpIndex_2,e_SpIndex_2,Curvature_2,e_Curvature_2,me1_2"
...
for c, cat in enumerate((np.compress(marco["primary"] == "nvss", marco), np.compress(marco["primary"] == "sumss_south", marco))):
    ...
    for source_id, fluxes, flux_errors in zip(cat["ID_1"], cat[flux_cols].view((np.float64, len(flux_cols))), cat[flux_error_cols].view((np.float64, len(flux_cols)))):
