...
marco = np.genfromtxt(INFILENAME, delimiter=',', names=True, usecols=usecols, dtype=None)
...
#
        ...
        plt.errorbar(freqs, fluxes,  yerr=flux_errors, fmt='o', color='k', label='Catalog flux')
        plt.plot(pltfreq, power_law(flux_ref1,  pltfreq/REF_FREQ, alpha1),  color='k',  label=r'$\nu_{408}$ %4.2f$\pm$%4.2f $\alpha$=%+5.2f$\pm$%4.2f' % (flux_ref1, flux_ref1_err, alpha1, alpha1_err))
        if curvature:
            plt.plot(pltfreq, power_law_curvature(flux_ref,  pltfreq/REF_FREQ, alpha, phi), ls='--', color='k',   label=r'$\nu_{408}$ %4.2f$\pm$%4.2f $\alpha$=%+5.2f$\pm$%5.2f $\phi$=%+5.2f$\pm$%5.2f' % (flux_ref, flux_ref_err, alpha, alpha_err, phi, phi_err))
        ...
        plt.ylabel("S_int/Jy")
        plt.axvspan(MIN, MAX, facecolor="#AAAAFF", alpha=0.25)
        plt.axvspan(MIN1, MAX1, facecolor="#FFAAAA", alpha=0.25)
        plt.savefig(os.path.join(OUTDIR, "%s.pdf" % name))
        #plt.show()
        plt.close()
